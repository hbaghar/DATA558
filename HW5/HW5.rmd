...
Let us generate the data matrix and clusters:

```{r}
set.seed(42)
mu <- c(0,5,-1)
var <- matrix(c(2, 0.8, 0.9, 0.8, 5, 1, 0.9, 1, 3), nrow = 3, ncol = 3)

cluster <- sample(seq(1,4), size = 100, replace = TRUE)

data <- cbind(mvrnorm(n = 100, mu = mu, Sigma = var), cluster)
```

Now let us compute the left-side of the given equality

```{r}

get.cluster.lhs <- function(c){
  sum <- 0
  x <- data[data[,4]==c,-c(4)]
  for(i in 1:nrow(x)){
    for(i1 in 1:nrow(x)){
      for(j in 1:ncol(x)){
        sum <- sum + (x[i,j]-x[i1,j])^2
      }
    }
  }
  return(sum)
}
(lhs <- sum(sapply(1:4, function(x) get.cluster.lhs(x)))/4)
```
...
(rhs <- sum(sapply(1:4, function(x) get.cluster.rhs(x)))*2)
```
...
Using the first to Principal components we see that there is an improvement in identifying the green cluster.
