...
The procedure of estimating test error in (c) has a smaller test error since we use the true form of $f$, moreover our estimated $\hat{f}$ is also based on properties of the standard normal distribution (mean value of $\hat{f}$ is 0). We also see that the test MSE is equal to the irreducible error and it is the lowest possible value of MSE we can achieve. Procedure in (d) will have a higher bias and variance, because we try to estimate $f$ using $\hat{f}$ from a linear model. 
...
```{r}
hist(cor(y,x))
sort(abs(cor(y,x)),decreasing = TRUE)[1:10]
```
